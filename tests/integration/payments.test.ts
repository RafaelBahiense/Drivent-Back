...
describe("POST /reservations", () => {
  it("should return the information of the payment of the user when sucessed to book the room", async () => {
    ...
    const response = await agent.post("/payments").send({
      value: 2000,
      reservationId: reservation.id
    }).set("Authorization", "Bearer " + session.token);

    expect(response.body).toEqual(expect.objectContaining({ date: expect.any(String), id: expect.any(Number), reservationId: expect.any(Number), value: expect.any(Number) }));
  });
});
